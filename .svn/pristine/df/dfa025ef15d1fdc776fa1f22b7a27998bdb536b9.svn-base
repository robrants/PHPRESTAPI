...
class firmWare extends CommonQueries{
	...
	private function upgradeONT($ONT){
		...
		sleep(20);
		$output1 = trim($output);
		$output1 = substr($output1,strlen($output1)-1,strlen($output1));
		$flag = 0;
		$loop = 0;
		while ($flag == 0 ){
			if($output1 != '>'){
				...
				*/
				...
				$loop++;
				$message = $loop.' Attempt to Log in';
		...
		if($flag == 1){			
			...
			if(trim($output) == '#'){				
				...
				if(count($m)>0){
					...
					$message = 'Upgrade of '.$ONT['IP']." Failed on archive download-sw. Please Investigate Logs for cause before running manually";					
					$this->msg->pushlog($message,__CLASS__,__METHOD__);
					unset($this->msg);
					return -1;}
				if(trim($output) == '#'){
					...
					$message = 'Failed to regain Admin Prompt after download - on device'.$ONT['IP'];
					$this->msg->pushlog($message,__CLASS__,__METHOD__);
					unset($this->msg);
					return -3;
				}
			}else{
				$message = 'Failed to Gain Administrative Control - Command en at device '.$ONT['IP'];
				$this->msg->pushlog($message,__CLASS__,__METHOD__);
				unset($this->msg);
				return -3;}
		}else {
			...
			$this->msg->pushlog($message,__CLASS__,__METHOD__);
			$message = 'Screen Output is '.$output;
			$this->msg->pushlog($message,__CLASS__,__METHOD__);
			unset($this->msg);
